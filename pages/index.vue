...
<template>
  <section class="pt-32 flex flex-wrap justify-between w-full">
    <div class="flex flex-wrap gap-2 w-full md:flex-nowrap">
      <ButtonsButtonText class="grow basis-full md:basis-0" v-for="button in buttons" :key="button.id"
        :text="button.text" @click="selectButton(button.id)" :disabled="button.disable" />
      <InputsSimpleInput class="grow basis-full md:basis-12/12" @getValue="(value) => updateFile(value)"
        :placeholder="store.getFile()" :value="file" />
      <ButtonsIconButton class="grow basis-1 px-6" icon="heroicons:arrow-path-16-solid" @click="resetColors()" />
    </div>
    <section class="mt-8 flex flex-wrap justify-between items-center gap-4">
      <section class="flex flex-col justify-between gap-4 grow basis-1/6 h-fit xl:h-full">
        <template v-for="color in store.colors" key="color.id">
          <div v-if="color.id === 5" class="flex justify-between items-center gap-4 w-full">
        ...
        <div class="flex w-full h-full rounded-xl bg-gray-200 dark:bg-gray-900"></div>
      </section>
      <section class="flex flex-col grow basis-full rounded-xl overflow-hidden xl:basis-1/6 h-80 xl:h-full">
        <div v-for="color in store.colors"
          ...
          :style="bgGen(color.value)" @click="copyColor(color.value)"></div>
      </section>
      <section class="w-full h-fit xl:h-full flex justify-center items-center grow basis-full xl:basis-3/6">
        <OtherPreviewTheme />
      </section>
    </section>
    <section class="flex flex-wrap justify-start items-center w-full h-full mt-20">
      ...
      <div v-for="profile in profiles" key="profile.id" class="mt-5 flex w-full justify-start items-start gap-8">
        <span class="w-1/6 text-xl text-gray-700 dark:text-gray-500">{{ profile.name }}</span>
        <div class="flex justify-start items-center gap-4">
          <ButtonsIconButton icon="heroicons:information-circle" />
          <ButtonsIconButton icon="heroicons:clipboard" />
          <ButtonsIconButton icon="heroicons:archive-box-arrow-down" />
        </div>
      </div>
    </section>
    <section class="flex flex-wrap justify-start items-center w-full h-full mt-32">
      ...
      <ListSimpleEnumList number="1">
        ...
        <template v-slot:description>Crear una carpeta para contener los temas creados y no mezclarlos con
      ...
      <ListSimpleEnumList number="2">
        ...
        <template v-slot:description>Crea el archivo del tema con extencion .toml en la carpeta
          creada.</template>
      </ListSimpleEnumList>
      <CodesSimpleCodeBlock class="my-8">
      ...
      <ListSimpleEnumList number="3">
        ...
        <template v-slot:description>Agrega las líneas de código generadas por la página de Terco_ en el
      ...
      <ListSimpleEnumList number="4">
        ...
        <template v-slot:description>Agrega la siguiente línea en la parte superior de tu archivo de
      ...
      <ListSimpleEnumList number="5">
        ...
        <template v-slot:description>Si el tema no se aplica, pueder cargar el archivo con el comando
