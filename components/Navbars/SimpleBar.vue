  ...
  <nav class="w-full flex flex-wrap justify-between items-center">
    <div class="flex justify-between items-center w-full">
      <div class="h-8 flex justify-center items-center gap-8">
        ...
        <h1 class="text-3xl sm:text-7xl md:text-6xl font-normal text-gray-900 dark:text-gray-200">
          TerCo
        </h1>
      </div>
      ...
      <div class="pt-32 flex flex-wrap justidy-start items-start w-full lg:hidden" v-show="showMenu">
        ...
        <ul class="flex flex-col">
          <li v-for="anchor in anchors">
            <a class="w-full text-gray-500 dark:text-gray-700 text-start text-xl sm:text-3xl transition-all duration-250 hover:text-blue-500 dark:hover:text-blue-500 hover:translate-x-4"
              :href="anchor.url">{{ anchor.name }}</a>
          </li>
        </ul>
      </div>
    </Transition>
